...
type GlyphRenderer = (p: GlyphStroke) => React.ReactNode;

function Strokes({ color, strokeWidth, children }: { readonly color: string; readonly strokeWidth: number; readonly children: React.ReactNode }) {
  return <>{children}</>;
}

const renderLibraryBookOpen: GlyphRenderer = (p) => (
  <>
...
const renderBrowseFolder: GlyphRenderer = (p) => (
  <Path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" {...p} />
);

const GLYPH_RENDERERS: Record<IconName, GlyphRenderer> = {
  library: renderLibraryBookOpen,
  bookOpen: renderLibraryBookOpen,
  book: (p) => (
    <>
  ...
  browse: renderBrowseFolder,
  folder: renderBrowseFolder,
  browser: (p) => (
    <>
  ...
  chevronDown: (p) => <Polyline points="6 9 12 15 18 9" {...p} />,
  grid: (p) => (
    <>
  ...
  play: (p) => <Polygon points="6 3 20 12 6 21 6 3" fill={p.stroke} stroke={p.stroke} strokeWidth={p.strokeWidth} strokeLinejoin="round" />,
  pause: (p) => (
    <>
      <Rect x="6" y="4" width="4" height="16" rx="1" fill={p.stroke} stroke="none" />
      <Rect x="14" y="4" width="4" height="16" rx="1" fill={p.stroke} stroke="none" />
    </>
  ),
  plus: (p) => (
    <>
  ...
  checkCircle: (p) => (
    <>
      <Circle cx="12" cy="12" r="10" fill={p.stroke} stroke="none" />
  ...
  circle: (p) => <Circle cx="12" cy="12" r="9.5" {...p} />,
  trash: (p) => (
    <>
  ...
  more: (p) => (
    <>
      <Circle cx="12" cy="5" r="1.7" fill={p.stroke} stroke="none" />
      <Circle cx="12" cy="12" r="1.7" fill={p.stroke} stroke="none" />
      <Circle cx="12" cy="19" r="1.7" fill={p.stroke} stroke="none" />
    </>
  ),
  ai: (p) => <Path d="M13 2 3 14h9l-1 8 10-12h-9l1-8z" {...p} />,
  sort: (p) => (
    <>
...
function Glyph({ name, color, strokeWidth }: { readonly name: IconName; readonly color: string; readonly strokeWidth: number }) {
  const p: GlyphStroke = { stroke: color, strokeWidth, fill: 'none', strokeLinecap: 'round', strokeLinejoin: 'round' };
  const render = GLYPH_RENDERERS[name];
  if (render) {
    return <>{render(p)}</>;
  }
